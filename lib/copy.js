        ...
        case CONFIG_MODE.FILE_ACTIVATE: {
            try {
                ...
                for (let i = 0; i < actives.length; ++i) {
                    ...
                    copys.push({
                        sourceDir: fromPrefix,
                        froms: fromItems.map(item => handleFrom(item)),
...
function handleCopy(val) {
    ...
    if (!sourceDirStat || !sourceDirStat.isDirectory()) {
        console.error(`${sourceDir} is not a dir!`);
        return;
    }
        ...
        if (isMatch) {
            if (isDir) {
                targets.forEach(item => {
                    ...
                    if (isDir) {
                        console.log(`copy dir ${chalk.green(_path)} to dir ${chalk.green(path.join(value, _name))}`);
                        copyDir(_path, path.join(value, _name));
                    } else {
                        console.warn(`cannot copy dir ${chalk.red(_path)} to file ${chalk.red(value)}`);
                    }
                })
            } else {
                targets.forEach(item => {
                    const [isDir, value] = item;
                    if (isDir) {
                        console.log(`copy file ${chalk.green(_path)} to dir ${chalk.green(value)}`);
                        copyFile(_path, path.join(value, _name));
                    } else {
                        console.log(`copy file ${chalk.green(_path)} to file ${chalk.green(value)}`);
